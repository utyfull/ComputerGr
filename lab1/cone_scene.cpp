...
bool ConeScene::Init(D3D12Core& core)
{
    ...
    // Root signature: b0 (камера), t0..t2 (SRV), b1 (ObjectCB как root-constants)
    ...
    // InstanceData buffer: 3 конуса, пол, 3 маркера
    ...
    // Dummy-запись для точечного света
    {
        PointLightCPU dummy{};
        void* p = nullptr;
    ...
    // Параметры прожекторов (CPU-структуры spot_)
    {
    ...
    // t0: gInstances
    {
        ...
        device->CreateShaderResourceView(instBuf.Get(), &srv, hCPU);
    }
    ...
    hCPU.ptr += srvInc;
    {
        ...
        device->CreateShaderResourceView(pointLightBuf.Get(), &srv, hCPU);
    }
    ...
    hCPU.ptr += srvInc;
    {
        ...
        device->CreateShaderResourceView(spotLightBuf.Get(), &srv, hCPU);
    }
...
void ConeScene::Render(D3D12Core& core, D3D12_CPU_DESCRIPTOR_HANDLE rtv)
{
    ...
    // Псевдопозиция направленного источника для маркера
    ...
    // Viewport под baseAspect_
    int   VW = core.Width();
    int   VH = core.Height();
    float currentAspect = (VH > 0) ? static_cast<float>(VW) / static_cast<float>(VH) : baseAspect_;
